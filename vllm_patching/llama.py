...
from hf_rocm_kernels.combined_ops.llama_mlp import llama_mlp_combined_ops
...
class LlamaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if (self.use_fp8) and (hidden_states.size(0) <= 8):
            hidden_states = llama_mlp_combined_ops(
                attn_output=hidden_states,
                residual=residual,
                rms_weight=self.post_attention_layernorm.weight,
                rms_epsilon=self.post_attention_layernorm.variance_epsilon,

                gate_up_weight=self.mlp.gate_up_proj.weight,
                gate_up_input_scale=self.mlp.gate_up_proj.input_scale,
                gate_up_output_scale=self.mlp.gate_up_proj.combined_scale,

                down_weight=self.mlp.down_proj.weight,
                down_input_scale=self.mlp.down_proj.input_scale,
                down_output_scale=self.mlp.down_proj.combined_scale,
...
class LlamaForCausalLM(nn.Module, SupportsLoRA, SupportsPP):
    ...
    def load_weights(self, weights: Iterable[Tuple[str,
        ...
        names = loader.load_weights(
            self.maybe_remap_mistral(name, loaded_weight)
            for name, loaded_weight in weights)
        # modify here some weights
        return names
