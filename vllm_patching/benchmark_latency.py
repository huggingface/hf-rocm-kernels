...
def save_to_pytorch_benchmark_format(args: argparse.Namespace, results: Dict[str, Any], pt_file: str) -> None:
...
class LLMBenchmarker:
    ...
    def profile(self, batch_size: int, input_len: int, output_len: int) -> None:
        ...
        profile_dir = self.results_dir / f"profile_bs{batch_size}_il{input_len}_ol{output_len}"
    ...
    def benchmark(self, batch_size: int, input_len: int, output_len: int) -> None:
        ...
        save_to_pytorch_benchmark_format(self.args, results, str(results_file).replace(".json", ".pytorch.json"))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num-iters", type=int, default=10, help="Number of iterations to run.")
    ...
    # Initialize LLMBenchmarker and warm up
    llm_benchmarker = LLMBenchmarker(args)
    llm_benchmarker.warmup(batch_size=args.batch_sizes[0], input_len=args.input_len, output_len=args.output_len)  
    llm_benchmarker.check_correctness(output_length=20)  

    # For each batch size, profile or benchmark
    for batch_size in args.batch_sizes:
        if args.profile:
            llm_benchmarker.profile(batch_size=batch_size, input_len=args.input_len, output_len=args.output_len)
        else:
            llm_benchmarker.benchmark(batch_size=batch_size, input_len=args.input_len, output_len=args.output_len)
