...
def _test_residual_rms(rows: int, cols: int, verbose: bool) -> Tuple[Tuple[float, float, float], ...]:
    ...
    qinput, attn_res = residual_rms(input, residual.clone(), weights, epsilon, scale.item())
    scale_ = scale * 2
...
def test_residual_rms(
    ...
    (max_error_qinput, max_relaive_error_qinput, changes_qinput), (max_error_res, _, _), (max_error_scale, _, _) = (
        _test_residual_rms(rows, cols, verbose=False)
    )
