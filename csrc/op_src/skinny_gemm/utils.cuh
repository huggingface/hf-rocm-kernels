...
#pragma unroll
    for (int i = 0; i < E_PER_BANK; i++) {
        reg[i + 0 * E_PER_BANK] = buffer[i + 0 * NB_BANKS * E_PER_BANK];
    }
#pragma unroll
    for (int i = 0; i < E_PER_BANK; i++) {
        reg[i + 1 * E_PER_BANK] = buffer[i + 1 * NB_BANKS * E_PER_BANK];
    }
}
...
#pragma unroll
    for (int i = 0; i < E_PER_BANK; i++) {
        reg[i + 2 * E_PER_BANK] = buffer[i + 2 * NB_BANKS * E_PER_BANK];
    }
#pragma unroll
    for (int i = 0; i < E_PER_BANK; i++) {
        reg[i + 3 * E_PER_BANK] = buffer[i + 3 * NB_BANKS * E_PER_BANK];
    }
}

//----------------------------------------------------------------------------------------------------------------------
template <int LOADS, bool REUSE, int OP_K>
void inline __device__ load_from_gmem_to_reg_no_waitcnt(const fp8*& src, fp8 reg[LOADS][16]) {
    if constexpr (LOADS == 1) {
        if constexpr (REUSE) {
            ...
            asm volatile("global_load_dwordx4 %0, %1, off offset:0  sc0 sc1 nt  \n\t" : "=v"(reg[0]) : "v"(src));
        }
    }
        ...
        if constexpr (OP_K == 16) {
            if constexpr (REUSE) {
                asm volatile(
                    ...
                    "global_load_dwordx4 %1, %2, off offset:32  \n\t"
                    : "=v"(reg[0]), "=v"(reg[1])
                    : "v"(src));
            } else {
                asm volatile(
                    "global_load_dwordx4 %0, %2, off offset:0   sc0 sc1 nt  \n\t"
                    "global_load_dwordx4 %1, %2, off offset:32  sc0 sc1 nt  \n\t"
                    : "=v"(reg[0]), "=v"(reg[1])
                    : "v"(src));
            }
        } else if constexpr (OP_K == 32) {
            if constexpr (REUSE) {
                asm volatile(
                    "global_load_dwordx4 %0, %2, off offset:0   \n\t"
            ...
            // Default case for other values of OP_K == 64
            if constexpr (REUSE) {
                asm volatile(
                    "global_load_dwordx4 %0, %2, off offset:0   \n\t"
                    "global_load_dwordx4 %1, %2, off offset:64  \n\t"
        ...
        if constexpr (OP_K == 16) {
            if constexpr (REUSE) {
                ...
                asm volatile(
                    ...
                    "global_load_dwordx4 %1, %4, off offset:32  sc0 sc1 nt  \n\t"
                    "global_load_dwordx4 %2, %4, off offset:64  sc0 sc1 nt  \n\t"
                    "global_load_dwordx4 %3, %4, off offset:96  sc0 sc1 nt  \n\t"
                    : "=v"(reg[0]), "=v"(reg[1]), "=v"(reg[2]), "=v"(reg[3])
                    : "v"(src));
            }
        } else if constexpr (OP_K == 32) {
            ...
            // Default case for other values of OP_K
            if constexpr (REUSE) {
                asm volatile(
                    "global_load_dwordx4 %0, %4, off offset:0    \n\t"
                    "global_load_dwordx4 %1, %4, off offset:64   \n\t"
