...
template <int LANES, int QSIZE,
          ...
          int OPS, bool REUSE>
void __device__
produce_n_full_tiles(const fp8* __restrict__ source, fp8* buffer, const int producers, int* queue, int& index,
                     int& p_state, int& role_id,
                     int dropped_ad,  // number of rows or columns that are out of bounds for this producer
                     const int stride_ad, const int k_blocks) {
...
#pragma unroll
        ...
        // if (lane_id == 0) {
        //     printf("threadIdx.x: %d, blockIdx.x: %d, LANES: %d, role_id: %d, b: %d, index: %d, p_state: %d,
        //     reg[0][0]: %f\n",
        //             threadIdx.x,     blockIdx.x,     LANES,     role_id,     b,     index,     p_state,
        //             DB_FP8_TO_FP32(reg[0][0]));
        // }
